	...
	<div class="container">
		...
		<div class="tabs">
			<a href="${pageContext.request.contextPath}/vendor/manageorder?status=all"
				...
				class="<c:choose><c:when test='${param.status == "Processing_1"  || param.status == "Processing_2"}'>tab active</c:when><c:otherwise>tab</c:otherwise></c:choose>">Chưa duyệt</a> <a
				...
				class="<c:choose><c:when test='${param.status == "In Transit_1" || param.status == "In Transit_2"}'>tab active</c:when><c:otherwise>tab</c:otherwise></c:choose>">Đã gửi cho đơn vị
				vận chuyển</a> <a
				...
				class="<c:choose><c:when test='${param.status == "Cancelled"}'>tab active</c:when><c:otherwise>tab</c:otherwise></c:choose>">Đơn hàng bị hủy</a>
		</div>
